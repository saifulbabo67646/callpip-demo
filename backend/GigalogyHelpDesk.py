...
class GigalogyAssistantFunction(agents.llm.FunctionContext):
...
async def entrypoint(ctx: JobContext):
    await ctx.connect()
    print("context", ctx.room)
    print(f"Connected to room: {ctx.room.name}")
    #here we need to query the database to get the agent details by agentId
    #and based on the agent we will retrieve the system prompt, LLM model, tts and stt settings
    ...
    async def follow_up_appointment(email: str):
        ...
        awaitREADME.md asyncio.create_task(_answer(status))
